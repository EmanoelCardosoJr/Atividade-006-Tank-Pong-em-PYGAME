...
class Ball:
    ...
    def limit_wall_collision(self, ball, ball_mx, ball_my, height, width):
        ...
        if ball.y < 0 + 80:
            ...
            if (
                    (ball.x == conf.screen_width + 100 or ball.x > conf.screen_width + 100) and
                    (ball.y == conf.screen_height + 100 or ball.y > conf.screen_height + 100)
            ):
                return ball_mx, ball_my
            else:
                ball.y = 0 + 80
    ...
    def ball_shoot_and_hit(
        ...
        if conf.shoot:
            ...
            conf.shoot = False
        if conf.hit:
            conf.ball_mx, conf.ball_my = self.end_ball(
                ball1
            )
            conf.timer_on = False
            conf.hit = False
        if conf.shoot2:
            conf.ball2_mx, conf.ball2_my = self.shoot2(
                ball2,
                tank2_sprite,
                tank2_x + 25 / 2,
                tank2_y + 28 / 2,
            ...
            conf.shoot2 = False
        if conf.hit2:
            conf.ball2_mx, conf.ball2_my = self.end_ball(
                ball2
            )
    ...
    def ball_draw(self, screen, ball1, ball2):
        if conf.timer_on:
